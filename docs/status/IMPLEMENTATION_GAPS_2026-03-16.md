...
| **Confidence Calibration** | "Outputs must reference source material or explicitly declare uncertainty" | Schemas support confidence_level (EXTRACTION, NORMALIZATION, GROUNDED) but agent outputs (Hypothesis, Critique, Proposal) do not include explicit confidence_rationale field showing _why_ confidence is high/low. Confidence is stated but not decomposable. | When hypothesis reports confidence_score=0.75, users cannot see: "3 supporting claims + 1 contradiction → 75% confidence". Metrics like "# supporting evidence" and "contradiction density" are computed but not exposed.                                      |
...
| **Confidence rationale**   | All agent outputs  | Confidence_score field exists       | No field showing _why_ confidence has this value (e.g., "2 supporting claims, 1 contradiction → 60% confidence"). Confidence is asserted, not derived. | Users cannot audit confidence; appears arbitrary         |
...
4. Documentation: "How to run ScholarOS fully offline"
...
**ScholarOS is architecturally sound but functionally incomplete.** The five-layer design is solid; deterministic services are well-structured; MCP tool interface is clean. However:
